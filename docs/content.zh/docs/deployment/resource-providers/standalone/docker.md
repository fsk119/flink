...
* `--jars` (optional): the paths of the job jar and any additional artifact 
  ...
  {{< hint warning >}}
  As the argument number of the `--jars` option is unlimited, be aware of how you pass job arguments.
  If there are any job arguments, make sure the `--jars` option is not the last one, e.g. `... --jars <job-jar> --job-classname <job-class> <job-arg1> <job-arg2>`.
  {{< /hint >}}
...
services:
  jobmanager:
    ...
    ports:
      - "8081:8081"
    command: standalone-job --job-classname com.job.ClassName ["--jars" "/path/to/user-artifact"] [--job-id <job id>] [--fromSavepoint /path/to/savepoint] [--allowNonRestoredState] [job arguments]
    volumes:
