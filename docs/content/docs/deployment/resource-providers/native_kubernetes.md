...
<span class="label label-info">Note</span> `local` schema is still supported. If you use `local` schema, the jar must be provided in the image or download by a init container as described in this [example](#example-of-pod-template).
...
Then use the config option `-Dkubernetes.service-account=flink-service-account` to configure the JobManager pods service account used to create and delete TaskManager pods and leader ConfigMaps.
