...
class FilterableSourceITCase extends StreamingTestBase {
  ...
  def testProjectWithWatermarkFilterPushdown(): Unit = {
    val myTableDataId = TestValuesTableFactory.registerData(TestData.data3WithTimestamp)
    val ddl =
      s"""
    ...
    val result =
      tEnv.sqlQuery(
        "select a, c from TableWithWatermark WHERE LOWER(c) = 'hello'"
      ).toAppendStream[Row]
