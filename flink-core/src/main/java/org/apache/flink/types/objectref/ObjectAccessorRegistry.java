...
public class ObjectAccessorRegistry implements Serializable {

    private final Map<CacheKey, FileSystemFactory> fileIOMap = new ConcurrentHashMap<>();

    public void configure(ReadableConfig config) {}

    public ObjectAccessor resolve(ObjectDescriptor descriptor) {
        Path path = new Path(descriptor.getURI());
        String scheme = path.toUri().getScheme();
        if (scheme == null) {
            return new FileSystemAccessor(FileSystem.getLocalFileSystem(), path);
        }
        return resolve0(path);
    }

    public ObjectAccessor resolve0(Path path) {
        CacheKey cacheKey = new CacheKey(path.toUri().getScheme(), path.toUri().getAuthority());
        return new FileSystemAccessor(
